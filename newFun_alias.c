...
int handle_alias_args(char **alias_args, list_t **alias_head, list_t **out_addrs)
{
	...
	for (i = 1; alias_args[i] != NULL; i++)
	{
		...
		if (is_set_alias(alias_args[i]))
			set_alias(alias_args[i]); /* Validate that the set format is fine */
		else /* Print errors */
		{
			status = -1;
...
void handle_aliases(char **commands, list_t **alias_head)
{
	...
	{
		str = curr->str;
		if (strncmp(commands[0], str, strleng(commands[0])) == 0 && str[strleng(commands[0])] == '=')
		{ /* the command is an alias */
			alias_len = strleng(&str[strleng(commands[0]) + 2]); /* +2 beacause of "='" chars */
			strncpy(tmp_buff, &str[strleng(commands[0]) + 2], alias_len - 1);
