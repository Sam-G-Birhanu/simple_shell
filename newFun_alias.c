...
void handle_aliases(char **commands, list_t **alias_head)
{
	...
	{
		str = curr->str;
		if (strncmp(commands[0], str, strlen(commands[0])) == 0 && str[strlen(commands[0])] == '=')
		{ /* the command is an alias */
			alias_len = strlen(&str[strlen(commands[0]) + 2]); /* +2 beacause of "='" chars */
			strncpy(tmp_buff, &str[strlen(commands[0]) + 2], alias_len - 1);
