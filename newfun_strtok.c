...
/**
 * is_delimiter - Evaluates if a char is a delimiter or not
 * @c: Char to evaluate
...
/**
 * split_string - Divides a string into tokens
...
char *split_string(char *str, char *delimiter)
{
	...
	{
		if (!is_delimiter(*curr_pos, delimiter))
		{
			tkn_start = curr_pos;
			while (*curr_pos != '\0' && !is_delimiter(*curr_pos, delimiter))
				curr_pos++;
